...
export default function AdminNewCandidateForm({ activeJobs, createCandidateAction }: { activeJobs: any[], createCandidateAction: (formData: FormData) => Promise<{ success: boolean; error?: string }> }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res.success) {
       ...
       console.error(err)
       // Prevent NEXT_REDIRECT from rendering as a red error flash
       if (err.message && err.message.includes('NEXT_REDIRECT')) {
         return; // Skip turning off loading state during redirect
       }
    ...
    <div className="max-w-4xl animate-fade-in mx-auto">
      ...
      <form onSubmit={handleSubmit} className="bg-primary-lighter p-8 rounded-2xl shadow-xl border border-border">
        ...
        <div className="mb-6 border-b pb-6">
          ...
          <select required name="jobId" className="block w-full rounded-lg bg-primary border border-border px-4 py-3 text-sm text-light focus:border-accent focus:ring-1 focus:ring-accent transition-all duration-200">
        ...
        <div className="mt-8 flex justify-end gap-4 border-t border-border pt-8">
          <a href="/candidates" className="rounded-lg border border-border bg-primary px-5 py-2.5 text-sm font-medium text-light hover:border-accent hover:text-accent transition-all duration-200 flex items-center">
            Cancel
          </a>
          <button
            type="submit"
            disabled={isSubmitting}
            className="rounded-lg bg-accent px-5 py-2.5 text-sm font-bold text-primary hover:bg-accent-hover hover:scale-[1.02] transition-all duration-200 shadow-[0_0_15px_rgba(170,255,0,0.2)] hover:shadow-[0_0_20px_rgba(170,255,0,0.4)] disabled:opacity-50"
          >
