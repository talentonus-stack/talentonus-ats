            ...
            <div key={job.id} className="bg-primary-lighter rounded-2xl border border-border shadow-lg p-5 flex flex-col gap-4 relative">
              <div className="flex justify-between items-start">
                <div className="pr-4 overflow-hidden">
                  ...
                  <p className="text-xs font-semibold text-muted mt-1 truncate" title={job.company?.name}>{job.company?.name || 'No Company'}</p>
                </div>
                <Link href={`/jobs/${job.id}/edit`} className="text-xs font-bold text-muted hover:text-accent transition-colors shrink-0">
                  Edit
                </Link>
              </div>
            ...
            <table className="w-full table-fixed divide-y divide-border">
              <thead className="bg-primary-lighter/50">
                <tr>
                  ...
                  <th className="w-[20%] px-4 py-4 text-left text-xs font-semibold text-muted uppercase tracking-wider">Location</th>
                  ...
                  <th className="w-[5%] px-4 py-4 text-right text-xs font-semibold text-muted uppercase tracking-wider">Actions</th>
                </tr>
              </thead>
                ...
                {jobs.map((job) => (
                  <tr key={job.id} className="hover:bg-primary/50 transition-colors group">
                    <td className="px-4 py-4 overflow-hidden">
                      ...
                      <div className="text-xs text-muted mt-1 truncate" title={job.company?.name}>{job.company?.name || 'No Company'}</div>
                    </td>
